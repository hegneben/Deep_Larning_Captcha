...
# Charset rekonstruieren (muss exakt dem Training entsprechen)
# fc.weight shape = (63, 256) → 62 Zeichen + 1 blank
# 62 = 26 Kleinbuchstaben + 26 Großbuchstaben + 10 Ziffern
chars    = string.ascii_lowercase + string.ascii_uppercase + string.digits  # 62 Zeichen
char2idx = {c: i for i, c in enumerate(chars)}
idx2char = {i: c for c, i in char2idx.items()}

blank_idx   = len(char2idx)   # = 62
num_classes = len(char2idx)   # = 62
print(f"Charset: {num_classes} Zeichen  |  blank_idx: {blank_idx}")
print(f"Zeichen: {chars}")

# =========================
# ARCHITEKTUR
# (muss exakt dem Training entsprechen)
# =========================
...
class CRNN_ResTransformer(nn.Module):
    def __init__(self, num_classes, d_model=256, nhead=8, num_layers=4, dropout=0.1):
        ...
        # Nach 3x stride-2: H=64→8, W=160→20
        self.proj    = nn.Linear(256 * 8, d_model)
    ...
    def forward(self, x):
        ...
        feat = feat.reshape(B, W, C * H)  # C*H = 256*8 = 2048      # (B, W, C*H)
...
# Checkpoint ist ein reines state_dict (nur Gewichte)
...
# PREPROCESSING
# =========================
def preprocess(img, size=(160, 64)):
...
def predict(image_path, use_beam=True, beam_width=5, show_images=True):
    ...
    if show_images:
        ...
        axes[1].set_title("Model Input (160x64)")
        axes[1].axis("off")
        plt.tight_layout()
        plt.show()

    tensor = transform(img_pre).unsqueeze(0).to(device)  # (1, 3, 64, 160)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 55)
    print("  Handschrift-Erkennung - ResNet + Transformer (CTC)")
    print("=" * 55)
    print("  [1] Einzelnes Bild analysieren")
    print("  [2] Alle Bilder in einem Ordner analysieren")
    print("=" * 55)

    mode = input("\nModus waehlen (1/2): ").strip()

    if mode == "1":
        file_name  = input("Bildname (z.B. handschrift.png): ").strip()
        image_path = os.path.join(base_path, file_name)
        predict(image_path, use_beam=True, beam_width=5, show_images=True)

    elif mode == "2":
        folder = input(f"Ordnerpfad (Enter fuer {base_path}): ").strip() or base_path
        files  = [
        ...
        if not files:
            ...
            predict_batch(files, use_beam=False)

    else:
        print("Ungueltige Auswahl. Bitte 1 oder 2 eingeben.")
